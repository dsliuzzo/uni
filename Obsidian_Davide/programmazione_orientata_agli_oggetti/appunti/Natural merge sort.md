...
Il natural merge sort è un tipo di merge sort che sfrutta le sequenze di elementi crescenti e ci permette di ordinare grandi quantità di dati senza riempire direttamente la memoria in utilizzo.
L'idea alla base è quella di utilizzare due file di supporto su cui verranno scritte in modo alternato le sequenze le sequenze crescenti, nel momento in cui troviamo un valore minore del precedente scriviamo la sequenza nel file opposto. Concluso questo passaggio riscriviamo gli elementi all'interno del file di origine confrontando le sequenze a coppie dai due file accessori (se una sequenza si conclude finiamo di copiare l'altra sequenza) e continuiamo così finché i file accessori saranno vuoti. Successivamente ripetiamo questi passaggi con sequenze sempre più grandi fino ad ottenere un file completamente ordinato.

- **Complessità**
  $$O(n\log_2 n)$$
  Questo algoritmo ha complessità logaritmica in quanto nel caso peggiore (gli elementi sono in ordine decrescente) le sequenze nei file accessori saranno inizialmente di dimensione 1 e si raddoppieranno ogni iterazione per un totale di $n$ volte
- **Code**
```
import java.io.*;  
import java.util.*;  
