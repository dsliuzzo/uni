...
>- $1-F$ porzione di codice che non può beneficiare del parallelismo
>
...
Empiricamente il `for()` che potrebbe sembrare infinito converge in un numero finito di tentativi.
## Semafori
...
class Semaphore {
	...
	private List threadSospesi;
	
	public Semaphore(int permessi) {
		contatore = permessi;
		threadSospesi = new List();
	}
	...
	public void release() {
		contatore++;
		if (contatore <= 0) {
			Thread t = threadSospesi.remove(<uno dei thread sospesi>);
			<risveglia t>
		}
	}
}
