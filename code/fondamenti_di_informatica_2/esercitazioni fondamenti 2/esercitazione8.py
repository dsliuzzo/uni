...
class Sistema:
    ...
    dipendenti = ListaConcatenata() #ok piene
    riunioni = ListaConcatenata()  #ok piene
                ...
                while not it2.finito():
                    ...
                    if not tutti_argomenti.contains(argomento):
                        ...
                        while not it3.finito():
                            ...
                            cont_corrente = it4.next()
                            indice += 1
                            if argomento_corrente == argomento:
                                contatore_argomenti.rimuovi(indice)
                                contatore_argomenti.inserisci(indice, cont_corrente+1)
    ...
    def dipendenti_piu_attivi(self, nome_dip):
        ...
        while not it2.finito():
            dipendente = it2.next()
            if dipendente != dip:
                somma_durate_riunioni_altro = self.calcola_durata_riunioni(dipendente)
                if somma_durate_riunioni_altro > somma_durate_riunioni:
                    argomenti_interesse_altro = dipendente.get_argomenti_interesse()
                    if self.esperto_tutti_argomenti(dipendente.get_argomenti_interesse(),
                                               argomenti):
    ...
    # cioè che argomenti_Y è sottoinsieme di argomenti_X
    def _esperto_tutti_argomenti(argomenti_X, argomenti_Y):
        it = Iteratore(argomenti_Y)
        while not it.finito():
            argomento = it.next()
            if not argomenti_X.contains(argomento):
                return False
        return True
    ...
    def argomenti_no(self, nome_dip, d):
        ...
        while not it.finito():
            arg = it.next()
            indice += 1
            if argomento_trattato_in_data(self, arg, d):
                indici_da_rimuovere.aggiungi_in_testa(indice) #ordinam. decrescente
    ...
    def argomento_trattato_in_data(self, argomento, d):
        it = Iteratore(self.riunioni)
        while not it.finito():
            riunione = it.next()
            if riunione.get_data() == d:
